...
public class VentanaPrincipal extends JFrame {
    ...
    private void iniciarTimerInactividad() {
        timerInactividad = new Timer(1000, null);
        final int[] segundosRestantes = {TIMEOUT_MINUTOS * 60};

        timerInactividad.addActionListener(e -> {
            segundosRestantes[0]--;
            ...
            if (lblTiempoRestante != null) {
                lblTiempoRestante.setText(String.format("Sesion: %d:%02d  ", min, seg));
                lblTiempoRestante.setForeground(segundosRestantes[0] <= 60
                    ? new Color(200, 60, 60)
                    : GRIS_TEXTO);
            }
    ...
    private void reiniciarTimer() {
        ...
        Timer reinicio = new Timer(1000, null);
        final int[] seg = {TIMEOUT_MINUTOS * 60};
        reinicio.addActionListener(e -> {
            seg[0]--;
            int min = seg[0] / 60;
            int s   = seg[0] % 60;
            if (lblTiempoRestante != null) {
                lblTiempoRestante.setText(String.format("Sesion: %d:%02d  ", min, s));
                lblTiempoRestante.setForeground(seg[0] <= 60
                    ? new Color(200, 60, 60)
                    : GRIS_TEXTO);
            }
            if (seg[0] <= 0) reinicio.stop();
        });
    ...
    private void registrarActividadUsuario() {
        AWTEventListener listener = event -> reiniciarTimer();
    ...
    private void cerrarSesionPorInactividad() {
        JOptionPane.showMessageDialog(this,
            "La sesion se cerro automaticamente por inactividad.",
            "Sesion expirada", JOptionPane.WARNING_MESSAGE);
        SistemaSesion.setUsuarioActual(null);
    ...
    private JPanel crearHeader() {
        ...
        JLabel nombre = new JLabel("  MediTurnos  -  Centro Medico San Rafael");
    ...
    private JPanel crearSidebarIzquierdo() {
        ...
        side.add(Box.createVerticalStrut(4));
        side.add(sideBtn("Cerrar sesion",        new Color(160, 60, 60)));
    ...
    private JButton sideBtn(String texto, Color acento) {
        ...
        btn.addActionListener(e -> {
            switch (texto) {
                ...
                case "Cerrar sesion"        -> cerrarSesionManual();
            }
        });
    ...
    private void cerrarSesionManual() {
    ...
    private JPanel crearBarraEstado() {
        ...
        JPanel derecha = new JPanel(new FlowLayout(FlowLayout.RIGHT, 0, 3));
        derecha.setOpaque(false);

        lblTiempoRestante = new JLabel(String.format("Sesion: %d:00  ", TIMEOUT_MINUTOS));
        lblTiempoRestante.setFont(new Font("Segoe UI", Font.PLAIN, 11));
        lblTiempoRestante.setForeground(GRIS_TEXTO);
